...
//controller
  ...
  //method register
  void register(
      String email, String username, String phonenumber, String password) {
    ...
    final response = ClientRequest.postData(url, postBody).then((value) {
      if (value["status"] == "OK") {
        ClientRequest.deleteData(
                MySettings.getUrl() + ("register/user/") + value["code"])
            .then((resp) {
          if (resp["status"] == "OK") {
            CoolAlert.show(
              context: context,
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => LoginPage(admin: false),
            ));
      } else {
            ...
            Padding(
                ...
                child: Container(
                  ...
                  child: ElevatedButton(
                    style: ButtonStyle(
                        fixedSize:
                            MaterialStatePropertyAll(const Size(110, 40)),
                        alignment: Alignment.center,
                        backgroundColor:
